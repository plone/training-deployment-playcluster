...
# playcluster build configuration file
...
linkcheck_ignore = [
    ...
    r"https://github.com/plone/training-deployment-playcluster/issues/new",  # requires auth
...
# No logo image: the navbar shows html_theme_options["logo"]["text"] instead.
...
html_theme_options = {
    ...
    "icon_links": [
        {
            "name": "GitHub",
            "url": "https://github.com/plone/training-deployment-playcluster",
    ...
    "repository_url": "https://github.com/plone/training-deployment-playcluster",
...
# html_context = {
#     "edit_page_url_template": "https://github.com/plone/training-deployment-playcluster/edit/main/docs/",
# }
...
ogp_site_name = "playcluster Documentation"
ogp_type = "website"
...
htmlhelp_basename = "playclusterDocumentation"
...
latex_documents = [
    (
        "index",
        "playclusterDocumentation.tex",
        "playcluster Documentation",
...
# the title page. Unset, so the PDF title page carries the title only.
# latex_logo = ""


# -- Options for PDF output (rinohtype) ---------------------------------------

# `make pdf` renders through rinohtype, a pure-Python PDF writer, so building
# the PDF needs no LaTeX toolchain at all. The `latex` target still emits LaTeX
# sources for anyone who wants to run them through pdflatex by hand.
#
# Same shape as latex_documents. Set explicitly: rinohtype falls back to
# latex_documents otherwise, and warns each time it does.
rinoh_documents = [
    {
        "doc": "index",
        "target": "playclusterDocumentation",
        "title": "playcluster Documentation",
        "subtitle": "Release {release}",
        "author": "collective community",
    },
]


def strip_meta_nodes(app, doctree):
    """Drop `meta` nodes before rinohtype walks the tree.

    Pages carry MyST `html_meta` frontmatter with keys such as
    `property=og:description`. Those become docutils `meta` nodes with a
    `property` attribute and no `name`, and rinohtype's handler indexes its
    lookup table with `name` unconditionally -- so it raises KeyError: None and
    the build dies. The tags matter only to the HTML build, so removing them
    for this one builder costs nothing.
    """
    if app.builder.name != "rinoh":
        return
    for node in list(doctree.findall(docutils.nodes.meta)):
...
def setup(app):
    ...
    # See strip_meta_nodes above -- rinohtype only.
    app.connect("doctree-read", strip_meta_nodes)
